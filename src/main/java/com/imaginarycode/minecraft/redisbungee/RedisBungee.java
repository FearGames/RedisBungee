...
public final class RedisBungee extends Plugin {
    ...
    public void onEnable() {
        ...
        if (pool != null) {
            ...
            integrityCheck = service.scheduleAtFixedRate(this::run, 0, 1, TimeUnit.MINUTES);
        }
    ...
    private void loadConfig() throws IOException, JedisConnectionException {
        ...
        final boolean useSSL = configuration.getBoolean("redis-ssl");
        ...
        if (redisServer != null && !redisServer.isEmpty()) {
            ...
            try (Jedis rsc = pool.getResource()) {
                ...
                FutureTask<Void> task2 = new FutureTask<>(() -> {
                    httpClient = new OkHttpClient.Builder().dispatcher(new Dispatcher(getExecutorService())).build();
        ...
        try (Jedis tmpRsc = pool.getResource()) {
            ...
            for (String member : absentLocally) {
                ...
                for (String proxyId : getServerIds()) {
                    if (proxyId.equals(configuration.getServerId())) {
                        continue;
                    }
